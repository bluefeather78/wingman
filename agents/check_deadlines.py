...
is where a silent call does the most visible damage. See wingman/gemini_common.py's SEVENTH finding.
...
USAGE:
    python -m agents.check_deadlines --sample 20   # random 20-row sample, prints measured cost
    python -m agents.check_deadlines --all         # full active catalog
"""
import argparse
import datetime
import json
...
from wingman.claude_common import estimate_cost
...
from wingman import sitemap_common
...
def check_one(opp, api_key, retry_on_silent=True, want_requirements=False):
    ...
    TWO CALLS, and the split is the accuracy design — see agents/check_reviews.py and
    wingman/gemini_common.py's SEVENTH finding. Demanding JSON collapses the search rate; measured
...
def main():
    ...
    # Same shape as the scraper's --seed-ids, and it pairs with wingman/clear_deadline_cache.py, which
    ...
    if args.dry_run:
        ...
        dry_path = os.path.join(REPO_ROOT,

...
Run far less often than agents/check_deadlines.py — an organization's reputation changes on the
...
1. TWO PHASES, BECAUSE ASKING FOR JSON IS WHY THIS AGENT NEVER SEARCHED. Until now it made
   ...
   See wingman/gemini_common.py's SEVENTH finding — including the limit on what it proves, because
...
2. A SILENT CALL WRITES NOTHING. The search decision is still non-deterministic and still
   cannot be forced (wingman/gemini_common.py's THIRD finding is correct; do not try again).
...
SETUP:
    ...
    (Also needs the agent_runs table — see agents/check_deadlines.py's docstring if not
    already created.)

USAGE:
    python -m agents.check_reviews --sample 20   # random 20-row sample of stale/unchecked rows
    python -m agents.check_reviews --all         # every stale/unchecked active row
    python -m agents.check_reviews --all --force # ignore the staleness filter, recheck everything
"""
import argparse
import datetime
import json
...
# an invented one. See the SEVENTH finding in wingman/gemini_common.py.
MAX_SEARCHES = 1
...
def build_research_system(opp):
    ...
    chunks against 0. See wingman/gemini_common.py's SEVENTH finding for the numbers and for the
...
def research_reviews(opp, api_key):
    ...
    (wingman/gemini_common.py's THIRD finding). A silent call is also the cheap one — it pays no
...
def check_one(opp, api_key):
    ...
    single-call JSON prompt. wingman/gemini_common.py's SEVENTH finding carries the numbers and the

...
Pilot with --limit first.
    ...
    python -m agents.classify_queue --preview            # FREE: row count + estimated cost
    python -m agents.classify_queue --limit 30           # PAID pilot (~$0.15): classify a sample
    python -m agents.classify_queue                      # PAID full queue (~$1, ~25 min), prints only
    python -m agents.classify_queue --limit 30 --write   # PAID pilot AND stamp each verdict onto the row
...
def _fetch_queue():
    ...
    # Service key REQUIRED: this reads the review queue itself (is_active=false). Under the
    # anon key RLS returns none of it and the job would report an empty queue (4.13).
...
def main():
    ...
    if hub_leads and not args.no_feed:
        ...
        print(f"  Mine them: python -m agents.mine_hub_pages --from-leads   (PAID, gated)")
    elif hub_leads:

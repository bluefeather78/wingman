...
Two independent selections feed the SAME embedding + dedupe_confidence tier engine (`--source`):
    ...
    flagged  the rows the catalog scan already flagged `suspected_duplicate` (Run -> Duplicates
             -> "Scan for duplicates" / wingman/find_catalog_dups.py). That scan's own verdict is
...
Supabase without --write — this only tells you what the logic WOULD judge.
    ...
    python -m agents.dedupe_queue                     # embeds the review queue, prints tier verdicts
    python -m agents.dedupe_queue --preview            # FREE: just the counts + estimated embed cost
    python -m agents.dedupe_queue --write              # ALSO stamps a dup_candidates back-link
    python -m agents.dedupe_queue --source flagged     # same, but over the suspected_duplicate rows
...
wingman/find_catalog_dups.py. It generates candidates over the WHOLE catalog cheaply (heuristic cuts,
...
from wingman.combined_reader import default_representation
...
def select_rows(rows, source, classified_only=False):
    ...
    agents/classify_queue.py has already labelled. 'flagged' is is_active=true rows the catalog scan
...
def _write_candidate(su, key, prow, survivor, tier, cosine):
    ...
    from wingman.supabase_common import supabase_patch
...
def main():
    ...
    ap.add_argument("--classified-only", action="store_true",
                    help="--source queue only: judge (and, with --write, stamp) only rows already "
                         "carrying a `classify:` flag -- i.e. the rows agents/classify_queue.py has "

...
Three properties of the old design made that inevitable, and this agent inverts all three:
  ...
  2. Nothing could check the answer. The proxy returns only the response text; grounding
     ...
     that cannot be supported is demoted or dropped. See wingman/page_text.py for the two tests.
...
--------------------------------------------
agents/check_reviews.py and agents/check_deadlines.py are two-phase because demanding JSON collapses the
...
from wingman.claude_common import call_claude, extract_json, estimate_cost
...
from agents import check_deadlines

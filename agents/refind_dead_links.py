...
Phase 4 of the scraper v2 plan. agents/check_links.py deactivated ~200 rows for dead links, and Phase 2
...
agent here — needs fresh explicit approval per run.
    ...
    python -m agents.refind_dead_links --preview            # FREE: which rows would be re-found
    python -m agents.refind_dead_links --limit 20            # PAID (gated): re-find up to 20 rows
"""
import argparse
import datetime
import os

...
Fields NEVER touched by this agent (reserved for other agents):
  ...
  - url, link_status, link_status_code, link_checked_at, link_dead_since (agents/check_links.py)
...
a constructed deep path. See docs/archive/SCRAPER_PLAN.md and wingman/url_validate.py.
...
So this agent does not write the field at all. `agents/check_links.py` owns link health: it verifies
...
SETUP:
    ...
    Writes to the shared `agent_runs` table (schema in agents/check_deadlines.py's docstring) as
    agent='metadata_refresher', so runs show up in the admin console alongside the other three.

USAGE:
    python -m agents.refresh_opportunities --preview      # resolve scope + count only, no API calls, free
    python -m agents.refresh_opportunities --sample 10    # random 10-row sample, prints projected cost
    python -m agents.refresh_opportunities --all          # every active row
    python -m agents.refresh_opportunities --dry-run      # calls the API but writes nothing; dumps JSON
"""
import argparse
import datetime
import json
...
def _reembed_row(supabase_url, service_key, merged_row, gemini_key):
    ...
    from wingman.dedupe_embed_store import refresh_row_dedupe_embedding        # scraper dedupe_vector
    cost = 0.0
...
def main():
    ...
    for i, opp in enumerate(items):
        ...
        try:
            ...
            # pass fills it without needing agents/find_contact_emails.py run separately.
    ...
    if args.dry_run:
        ...
        refresh_path = os.path.join(REPO_ROOT,

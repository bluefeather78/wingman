...
time (see wingman/gemini_common.py's FIFTH finding). It cannot be forced. So phase 1 RETRIES once on
...
DEDUP-based discard rule here without reading wingman/url_dedupe.py's measurements first.
...
USAGE:
    python -m agents.scrape_opportunities --mode seattle --preview    # free, resolves scope only
    python -m agents.scrape_opportunities --mode seattle --dry-run    # PAID, no DB writes
    python -m agents.scrape_opportunities --mode national             # real run
"""
import argparse
import datetime
import json
...
def main():
    ...
    for seed in seeds:
        ...
        try:
            ...
            for candidate, url, flags in staged:
                ...
                # wingman/url_dedupe.py's measurements show URL-alone rejects shared application portals
                ...
                # already set. This is why the standalone agents/find_contact_emails.py is only a
                ...
                # it becomes live feedback on this seed (wingman/seed_ledger.py). None for fallback
    ...
    if captured_leads:
        ...
        print("     queue now: "
              ...
              + "  (agents/mine_hub_pages.py --from-leads / agents/harvest_names.py --from-leads)")

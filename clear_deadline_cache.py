...
until 2026-08-24 - a name that only ever existed in check_deadlines.py's DDL comment - so
...
USAGE:
    python clear_deadline_cache.py ec12081                 # one row
    python clear_deadline_cache.py ec12081 us1787532028    # several
    python clear_deadline_cache.py --stale-only ec12081    # skip rows already due
    python clear_deadline_cache.py --all --yes-really      # every active row
    python clear_deadline_cache.py --all --dry-run         # count only, no writes
"""
import argparse
import datetime
import os

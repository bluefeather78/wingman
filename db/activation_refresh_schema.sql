...
-- not yet run through refresh_opportunities.py", so the console's Core Details tab can
...
-- activation and CLEARED the first time refresh_opportunities.py successfully reads the
...
--     succeeds.
--   * refresh_opportunities.py drops the column from its SELECT on a 400 and skips the

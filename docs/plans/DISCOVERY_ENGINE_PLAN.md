...
## Status note (added 2026-09-05, Phase 3 branch cleanup)

Moved here from the repo root of the `local-discovery-engine` branch, which was written before
the 2026-09-04 `agents/` + `wingman/` reorganisation. **The plan is preserved; the prototype is
not merged.**

That branch turned out to hold only three things main did not already have. Everything else on
it — the url_dedupe weak-tier suppression, the check_links discontinuation detection, the eval
golden set and scorecard — reached main by other routes and in a **newer** form, so a plain
`git merge` would have REVERTED the whole `wingman/` package layout along with them. It was
therefore never merged as a branch.

What is left un-ported is `local_org_discovery.py` — the stage 0 + stage 1 prototype this plan
describes. Its own header declares it **MARQUEE M8** (three prompts sent to a model) and
**MARQUEE M9** (a paid API call path), so bringing it onto main is a marquee action needing
Shama's approval first and its own dedicated commit. It also needs porting work before it would
run at all: it imports `url_dedupe` / `url_validate` / `supabase_common` / `agent_common` by
bare name (pre-reorg), belongs under `agents/`, and would have to pick up the Phase 3 rules
every other inserting agent now follows — the catalog run lock, `require_service_key()`, and
ordered pagination.

The branch is retained on `origin` (not deleted with the merged ones) so the prototype stays
recoverable until that decision is made.

...
- **`.env`** and the embedding index **`catalog_embeddings.jsonl`** (1,676 rows, active catalog) both
  ...
  (`.env` copied in; `python -m agents.build_catalog_embeddings --commit` to rebuild the index).
...
| `51d8d3a` | ✅ pushed | **MARQUEE M9** — discovery gate wired always-on into `agents/scrape_opportunities.py` (classify + metadata enrich + embedding dedupe per candidate) |
| `e5b029a` | ✅ pushed | `agents/triage_queue.py` — bulk-reject the queue by classifier verdict (FREE, reversible) |
...
- **Discovery gate** (`wingman/combined_reader.py` + `wingman/classify_page.py` + `wingman/dedupe_confidence.py` +
  `wingman/embed_common.py`), wired into the scraper's candidate loop. Per candidate: fetch page once →
  classify → (if program) enrich metadata + embedding-dedupe hint. Pure shaping helpers
  (`gate_metadata_overlay`, `gate_dup_candidates` in `agents/scrape_opportunities.py`) are unit-tested.
- **Queue tools** — surfaced in the console via the existing `MAINTENANCE_TOOLS` registry (NOT the
  ...
  tools**: `agents/classify_queue.py`, `agents/dedupe_queue.py`, `agents/triage_queue.py`, `agents/build_catalog_embeddings.py`,
...
- Full-queue **classify** (`agents/classify_queue.py --write`): 279 rows, **$0.4631** → 159 program, 38+18
  hub, 20 none, 44 unreadable, 11 stale.
- Full-queue **dedupe** (`agents/dedupe_queue.py --write`): 279 rows, **$0.0036** → 4 confident, 4 adjudicate,
  10 hints.
...
- **Hub mining is a SEPARATE pipeline from the gate.** `agents/mine_hub_pages.py` does NOT run
...
**Open decision the user was about to make:** `agents/classify_queue.py --write` re-classifies (re-pays for)
...
- The concurrent **scraper-v2** session owns `agents/mine_hub_pages.py` on `origin/main`; coordinate before
...
- **M1** — `agents/refresh_opportunities.py` reads the live page; `combined_reader` only calls its public

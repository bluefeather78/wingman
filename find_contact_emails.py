...
See contact_email_common.py for how a row is resolved (regex-first, one cheap model call
...
refresh_opportunities.py calls contact_email_common.resolve_contact_email() per row on its
...
USAGE:
    ...
    python find_contact_emails.py --limit 10             # a pilot pass
    python find_contact_emails.py --all                  # every active row missing contact_email
    python find_contact_emails.py --ids ec17081,ab12cd3  # named rows, e.g. a re-check
    python find_contact_emails.py --force                # re-check rows that already have one
    python find_contact_emails.py --dry-run --all         # calls the API, writes nothing
"""
import argparse
import datetime
import json

...
export default function Finder() {
  ...
  async function addOneToTracker(
    ...
    try {
      ...
      // The catalog's own checklist, generated by generate_action_items.py and verified
      // against the program's page, is authoritative over anything the model just wrote
      // here — same relationship the shared deadline check has to this call's dates, and
      // for a stronger reason: nothing on this path can read the page at all. The model's
      // items are the fallback for a row the agent has not reached, and they are forced
      // generic because nothing verified them.
      const shared = await httpClient.getActionItems(opp.id);
      const sharedItems = normalizeVerifiedActionItems(shared?.action_items, opp.id);
      const res = await addTrackerItemChecked(bucket, {
        ...
        actionItems: sharedItems.length
          ? sharedItems
          : normalizeUnverifiedActionItems(info.action_items, opp.id),
      });

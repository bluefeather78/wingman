...
Once `classify_queue.py --write` (or the scraper's discovery gate) has stamped `classify:` flags
...
own moderation endpoint, so there is no logic drift.
    ...
    python triage_queue.py                       # FREE: full class breakdown, writes nothing
    python triage_queue.py --all-junk --dry-run  # FREE: show exactly what --all-junk would reject
    python triage_queue.py --all-junk            # reject hubs + none + stale programs
    python triage_queue.py --reject-hubs         # reject only the hub rows

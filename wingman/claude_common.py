...
agents/check_deadlines.py) that call the Anthropic Messages API directly (not through
...
# Haiku, matching server.py's CLAUDE_MODEL and agents/check_deadlines.py's own pin. This module
...
# Rate limiting. This did NOT exist until now: agents/check_deadlines.py carried a comment
...
# agents/check_deadlines.py's --min-delay defaults to 5 and applies it at startup; the
# interactive path leaves it at 0 and is unaffected.
DEFAULT_MIN_DELAY_SECS = 0
BATCH_MIN_DELAY_SECS = 5  # what agents/check_deadlines.py's --min-delay defaults to

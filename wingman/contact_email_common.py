...
Shared by agents/find_contact_emails.py (the standalone pass over the whole catalog) and
agents/refresh_opportunities.py (which calls resolve_contact_email() per row so a plain metadata
...
WHY THIS IS ALMOST ALL REGEX, NOT AI (same argument wingman/mailing_list_common.py makes for
...
Step 5 exists for the same reason agents/find_mailing_lists.py needs a model at all: a page can
...
Gemini means it uses the key its only two callers ALREADY require (agents/refresh_opportunities.py
and agents/find_contact_emails.py both need GEMINI_API_KEY), so multi-candidate rows now resolve
...
from wingman.gemini_common import call_gemini, estimate_cost, extract_json
from wingman.mailing_list_common import fetch_page  # reused fetch plumbing

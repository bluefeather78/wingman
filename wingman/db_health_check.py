...
is no approval to seek before running it.
    ...
    python -m wingman.db_health_check            # print the report
    python -m wingman.db_health_check --json     # emit the same data as one JSON object (for tooling)
...
# Staleness thresholds. STALE_AFTER_DAYS matches agents/check_reviews.py's own 30-day filter so this

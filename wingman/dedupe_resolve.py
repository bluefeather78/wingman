...
Usage:
    python -m wingman.dedupe_resolve                 # preview: histogram + sample, writes nothing
    python -m wingman.dedupe_resolve --write         # ALSO PATCH dup_verdict (null when no duplicate)
    python -m wingman.dedupe_resolve --limit 200     # first N rows (debugging)
    python -m wingman.dedupe_resolve --id ec18702    # inspect one row's resolution in detail
"""
import argparse
import json
import os
...
from wingman.dedupe_embed_store import fetch_dedupe_index

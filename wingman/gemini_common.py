#!/usr/bin/env python3
"""Shared helper for wingman's offline agent scripts (agents/scrape_opportunities.py,
agents/check_deadlines.py, agents/check_reviews.py) that call the Gemini API directly (not through
server.py's /api/messages proxy, since these run standalone, off the dev server).

Replaces wingman/claude_common.py as of the Gemini migration (2026-08-18) — same public shape
(`call_gemini`/`extract_json`/`estimate_cost`) so the three calling scripts only needed
their import line and api-key env var swapped, not their call sites. wingman/claude_common.py
...
This is LOW risk for agents/scrape_opportunities.py (worst case: a recalled-from-memory candidate is
...
This is a HIGHER risk for agents/check_deadlines.py and agents/check_reviews.py specifically, because their
...
is what agents/check_deadlines.py/check_reviews.py/scrape_opportunities.py already do: check
...
outright NON-DETERMINISTIC. agents/scrape_opportunities.py seed 51 was run twice with an identical
...
wingman/url_validate.py for resolving and validating. Do not "simplify" that away.
...
That is consistent with everything else on record. `agents/check_reviews.py`, whose prompt demands
...
def call_gemini(system, user_content, api_key, use_web_search=True, max_tokens=4000, timeout=None,
    ...
    silently truncated agents/check_reviews.py's review_summary/review_sources at max_tokens=700:
    ...
    from 673 to ~90-95 on both a trivial prompt and a real agents/check_reviews.py-shaped prompt
    ...
    job. Defaults to None (use MODEL) so agents/scrape_opportunities.py/check_deadlines.py/
    agents/check_reviews.py are unaffected."""

...
agents/generate_action_items.py (batch) and app/services/action_items.py (on-demand), so the
...
already, in wingman/url_repair.py ("accept on proof, not similarity") and in the scraper's
...
----------------------------------
No fuzzy matching, no similarity ratio, no stemming beyond a plural 's'. wingman/url_repair.py
...
def fetch_page_text_resolved(url, timeout=DEFAULT_TIMEOUT, allow_browser=False):
    ...
    client. agents/check_links.py measured ~9% of this catalog 403ing a non-browser agent plus 41
    ...
    agents (agents/refresh_opportunities.py).
    """
...
# is agents/refresh_opportunities.py, a single-threaded batch loop. The on-demand server path never
...
def distinctive_tokens(task_text, name="", org=""):
    ...
    The program's own name and organization are subtracted for the reason wingman/url_repair.py

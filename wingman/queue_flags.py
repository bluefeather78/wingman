...
produce a verdict per pending row, but `agents/classify_queue.py` / `agents/dedupe_queue.py` are read-only
...
# agents/check_links.py: act on real evidence the program is over, never on its silence. (This is why the

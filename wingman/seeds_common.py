...
These used to be hardcoded Python lists in agents/scrape_opportunities.py, which meant changing
...
FALLBACK: the original literals stay in agents/scrape_opportunities.py and are used if the table
...
def load_seeds(supabase_url, service_key, mode, fallback=None, include_disabled=False):
    ...
    if fallback:
        print(f"[WARN] scraper_seeds returned no rows for mode={mode} — falling back to the "
              f"{len(fallback)} hardcoded seed(s) in agents/scrape_opportunities.py. Per-seed yield "

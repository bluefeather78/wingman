...
Why this exists separately from the `normalize_url()` that agents/scrape_opportunities.py,
wingman/dryrun_common.py and scripts/one-off/migrate_to_supabase.py each carry: those three are deliberately
identical to one another (see the note in wingman/dryrun_common.py) and changing them would change

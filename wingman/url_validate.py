...
check_urls()
    ...
    at all — a 404 here is highly reproducible, which is what makes agents/check_links.py's
...
see agents/scrape_opportunities.py's FLAG_* constants and docs/archive/SCRAPER_PLAN.md's "discard almost
...
def check_url(url, timeout=DEFAULT_TIMEOUT):
    ...
    short and stable — agents/check_links.py writes it to opportunities.link_status_code and the

...
The router (`wingman/discovered_leads.py`) finds round-ups because they link OUTWARD: >= 6 distinct
...
precisely the links that did not qualify it - for a round-up, its own menu.
    ...
    python -m wingman.walk_up_hubs                  # FREE: preview what the whole active catalog implies
    python -m wingman.walk_up_hubs --limit 40       # FREE: only the 40 best-ranked parents
    python -m wingman.walk_up_hubs --commit         # FREE: write them as leads for hub mining

Every tier here is free - it is plain HTTP and pure code, with no model call anywhere. The PAID
step is mining what this queues, which is gated as usual (`agents/mine_hub_pages.py --from-leads`).
"""
import argparse
...
from wingman.agent_common import safe_console
...
def main():
    ...
    if args.commit:
        ...
        print("     Mining them is PAID and gated: python -m agents.mine_hub_pages --from-leads")
    else:
        print("")
